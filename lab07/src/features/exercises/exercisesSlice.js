...
export const exercisesSlice = createSlice({
  ...
  reducers: {
    ...
    addEx: (state, action) => {
      const {id, exercise, weight} = action.payload;
      state.allEx.push({id: id, exercise: exercise, weight: weight});
    },
    updateSingleEx: (state, action) => {
      const {id, exercise, weight, atIndex} = action.payload;
      state.allEx.splice(atIndex, 0, {id: id, exercise: exercise, weight: weight});
    },
